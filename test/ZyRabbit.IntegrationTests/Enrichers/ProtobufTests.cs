...
namespace ZyRabbit.IntegrationTests.Enrichers
{
	public class ProtobufTests
	{
		...
		{
			using (var client = ZyRabbitFactory.CreateTestClient(new ZyRabbitOptions{ Plugins = p => p.UseProtobuf() }))
			{
				/** Setup **/
		...
		{
			using (var protobufClient = ZyRabbitFactory.CreateTestClient(new ZyRabbitOptions { Plugins = p => p.UseProtobuf() }))
