...
namespace ZyRabbit.IntegrationTests.Enrichers
{
	public class AttributeEnricherTests
	{
		...
		{
			using (var publisher = ZyRabbitFactory.CreateTestClient(new ZyRabbitOptions { Plugins = plugin => plugin.UseAttributeRouting() }))
		...
		{
			using (var subscriber = ZyRabbitFactory.CreateTestClient(new ZyRabbitOptions { Plugins = plugin => plugin.UseAttributeRouting() }))
		...
		{
			using (var responder = ZyRabbitFactory.CreateTestClient(new ZyRabbitOptions { Plugins = plugin => plugin.UseAttributeRouting() }))
		...
		{
			using (var responder = ZyRabbitFactory.CreateTestClient(new ZyRabbitOptions { Plugins = plugin => plugin.UseAttributeRouting() }))
			using (var requester = ZyRabbitFactory.CreateTestClient(new ZyRabbitOptions { Plugins = plugin => plugin.UseAttributeRouting() }))
			{
				/* Setup */
				var receivedTcs = new TaskCompletionSource<AttributedRequest>();
				await responder.RespondAsync<AttributedRequest, AttributedResponse>(received =>
