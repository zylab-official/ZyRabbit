...
namespace ZyRabbit.IntegrationTests.MessageSequence
{
	public class MessageSequenceTests
	{
		...
		{
			/* Setup */
			var cfg = ZyRabbitConfiguration.Local;
		...
		public async Task Should_Forward_Message_Context_In_When_Message_Handler()
		{
			...
			using (var sequenceClient = ZyRabbitFactory.CreateTestClient(new ZyRabbitOptions
			{
				Plugins = p => p
					.UseStateMachine()
