...
using ZyRabbit.IntegrationTests.TestMessages.Extras;
using Xunit;
...
namespace ZyRabbit.IntegrationTests.PublishAndSubscribe
{
	public class AcknowledgementSubscribeTests
	{
		...
		{
			using (var publisher = ZyRabbitFactory.CreateTestClient())
			using (var firstSubscriber = ZyRabbitFactory.CreateTestClient(new ZyRabbitOptions { Plugins = p => p.UseRetryLater()}))
			using (var secondSubscriber = ZyRabbitFactory.CreateTestClient())
			{
				/* Setup */
